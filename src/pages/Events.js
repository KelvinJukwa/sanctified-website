...
const categories = ['All', 'Concert', 'Rally', 'Outreach', 'Worship Night', 'General'];
...
export default function Events() {
  ...
  return (
    <div style={{ paddingTop: '75px', minHeight: '100vh', background: 'var(--deep)' }}>
      ...
      {/* PAGE HEADER */}
      <section style={{ padding: '5rem 2rem 4rem', background: 'radial-gradient(ellipse at top, rgba(201,168,76,0.1) 0%, transparent 60%), var(--dark)', textAlign: 'center', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem', marginBottom: '1.25rem' }}>
          <div style={{ width: '40px', height: '1px', background: 'var(--gold)' }} />
          <span style={{ fontFamily: 'Cinzel, serif', fontSize: '0.65rem', letterSpacing: '0.5em', color: 'var(--gold)' }}>COME WORSHIP WITH US</span>
          <div style={{ width: '40px', height: '1px', background: 'var(--gold)' }} />
        </div>
        <h1 style={{ fontSize: 'clamp(2.5rem, 5vw, 4.5rem)', color: 'var(--text)', marginBottom: '1rem', lineHeight: 1.1 }}>Events & Concerts</h1>
      ...
      <div style={{ maxWidth: '1300px', margin: '0 auto', padding: '4rem 2rem 0' }}>
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '5rem' }}>
            <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.7rem', letterSpacing: '0.4em', color: 'var(--gold)' }}>LOADING EVENTS...</div>
          </div>
        )}

        {!loading && events.length === 0 && (
          <div style={{ textAlign: 'center', padding: '5rem 2rem', border: '1px dashed var(--border)', marginBottom: '4rem' }}>
            <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>📅</div>
            <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.7rem', letterSpacing: '0.3em', color: 'var(--gold)', marginBottom: '0.75rem' }}>NO EVENTS YET</div>
            <p style={{ color: 'var(--muted)', fontStyle: 'italic' }}>Upcoming events will appear here once added through the admin portal.</p>
          </div>
        )}
        ...
        {!loading && featured.length > 0 && (
          <div style={{ marginBottom: '5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
              ...
              <span style={{ fontFamily: 'Cinzel, serif', fontSize: '0.65rem', letterSpacing: '0.4em', color: 'var(--gold)' }}>FEATURED CONCERTS</span>
            </div>
                ...
                return (
                  <div key={event.id} onClick={() => setSelectedEvent(event)}
                    ...
                    <div style={{ position: 'relative', paddingBottom: '52%', overflow: 'hidden' }}>
                      {event.image_url ? (
                        <img src={event.image_url} alt={event.title} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.5s ease' }}
                          onMouseOver={(e) => (e.target.style.transform = 'scale(1.05)')}
                          onMouseOut={(e) => (e.target.style.transform = 'scale(1)')} />
                      ) : (
                        <div style={{ position: 'absolute', inset: 0, background: 'var(--dark)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      ...
                      {event.tickets_available && (
                        <div style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'var(--gold)', padding: '4px 12px', fontFamily: 'Cinzel, serif', fontSize: '0.58rem', letterSpacing: '0.15em', color: 'var(--deep)', fontWeight: 700 }}>TICKETS AVAILABLE</div>
                      )}
                    </div>
                    <div style={{ padding: '1.5rem', borderTop: '2px solid var(--gold)' }}>
                      <div style={{ display: 'inline-block', marginBottom: '0.6rem', padding: '2px 10px', background: categoryColors[event.category] || 'rgba(201,168,76,0.1)', border: `1px solid ${categoryBorders[event.category] || 'rgba(201,168,76,0.3)'}`, fontFamily: 'Cinzel, serif', fontSize: '0.58rem', letterSpacing: '0.18em', color: 'var(--gold)' }}>{event.category}</div>
                      <h3 style={{ color: 'var(--text)', fontSize: '1.2rem', marginBottom: '0.5rem', lineHeight: 1.3 }}>{event.title}</h3>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '1rem' }}>
                        {event.time && <span style={{ color: 'var(--muted)', fontSize: '0.85rem' }}>🕐 {event.time}</span>}
                        {event.venue && <span style={{ color: 'var(--muted)', fontSize: '0.85rem' }}>📍 {event.venue}</span>}
                      </div>
                      {event.description && <p style={{ color: 'var(--muted)', fontSize: '0.9rem', lineHeight: 1.7, fontStyle: 'italic', marginBottom: '1.25rem' }}>{event.description}</p>}
                      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                        ...
                        {event.tickets_available && event.ticket_url && (
                          <TicketLink url={event.ticket_url} style={{ padding: '0.6rem 1.5rem', background: 'var(--gold)', border: '1px solid var(--gold)', color: 'var(--deep)', textDecoration: 'none', fontFamily: 'Cinzel, serif', fontSize: '0.62rem', letterSpacing: '0.15em', fontWeight: 700, transition: 'all 0.3s', display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
                            🎟 Get Tickets
                          </TicketLink>
                        )}
                      </div>
        ...
        {!loading && events.length > 0 && <div className="gold-line" style={{ marginBottom: '4rem' }} />}
          ...
          <div style={{ marginBottom: '5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[...filtered].sort((a, b) => new Date(a.date) - new Date(b.date)).map((event) => {
                const d = formatDate(event.date);
                return (
                  <div key={event.id} onClick={() => setSelectedEvent(event)} className="event-row"
                    ...
                    <div style={{ padding: '1.25rem 1.5rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
                        <div style={{ padding: '2px 10px', background: categoryColors[event.category] || 'rgba(201,168,76,0.08)', border: `1px solid ${categoryBorders[event.category] || 'rgba(201,168,76,0.2)'}`, fontFamily: 'Cinzel, serif', fontSize: '0.55rem', letterSpacing: '0.15em', color: 'var(--gold)' }}>{event.category}</div>
                        {event.tickets_available && (
                          <div style={{ padding: '2px 10px', background: 'rgba(201,168,76,0.12)', border: '1px solid rgba(201,168,76,0.4)', fontFamily: 'Cinzel, serif', fontSize: '0.55rem', letterSpacing: '0.15em', color: 'var(--gold)' }}>🎟 TICKETS AVAILABLE</div>
                        )}
                      </div>
                      <h3 style={{ color: 'var(--text)', fontSize: '1.05rem', marginBottom: '0.4rem' }}>{event.title}</h3>
                      <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
                        {event.time && <span style={{ color: 'var(--muted)', fontSize: '0.82rem' }}>🕐 {event.time}</span>}
                        {event.venue && <span style={{ color: 'var(--muted)', fontSize: '0.82rem' }}>📍 {event.venue}</span>}
                      </div>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1rem', borderLeft: '1px solid var(--border)' }}>
                      {event.tickets_available && event.ticket_url ? (
                        <TicketLink url={event.ticket_url} style={{ padding: '0.55rem 1.2rem', background: 'var(--gold)', color: 'var(--deep)', textDecoration: 'none', fontFamily: 'Cinzel, serif', fontSize: '0.6rem', letterSpacing: '0.15em', fontWeight: 700, whiteSpace: 'nowrap', transition: 'all 0.3s' }}>
                          Get Tickets
                        </TicketLink>
                      ) : (
                        <span style={{ fontFamily: 'Cinzel, serif', fontSize: '0.58rem', letterSpacing: '0.1em', color: 'var(--muted)', textAlign: 'center', padding: '0 0.5rem' }}>Free Entry</span>
                      )}
                    </div>
                  </div>
                );
      ...
      {/* EVENT MODAL */}
      {selectedEvent && (
        <div onClick={() => setSelectedEvent(null)} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(8,8,16,0.93)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', overflowY: 'auto' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: 'var(--card)', border: '1px solid rgba(201,168,76,0.3)', maxWidth: '680px', width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
            <button onClick={() => setSelectedEvent(null)} style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'rgba(201,168,76,0.1)', border: '1px solid var(--border)', color: 'var(--gold)', cursor: 'pointer', width: '36px', height: '36px', fontSize: '1rem', zIndex: 10 }}>✕</button>
            <div style={{ position: 'relative', paddingBottom: '45%', overflow: 'hidden' }}>
              {selectedEvent.image_url ? (
                <img src={selectedEvent.image_url} alt={selectedEvent.title} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                <div style={{ position: 'absolute', inset: 0, background: 'var(--dark)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ position: 'absolute', bottom: '1.5rem', left: '2rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <div style={{ background: 'rgba(8,8,16,0.85)', border: '1px solid rgba(201,168,76,0.5)', padding: '0.75rem 1.25rem', textAlign: 'center', backdropFilter: 'blur(8px)' }}>
                  <div style={{ fontFamily: 'Cinzel, serif', fontSize: '2rem', fontWeight: 700, color: 'var(--gold)', lineHeight: 1 }}>{formatDate(selectedEvent.date).day}</div>
                  <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.6rem', letterSpacing: '0.2em', color: 'var(--muted)', marginTop: '3px' }}>{formatDate(selectedEvent.date).month} {formatDate(selectedEvent.date).year}</div>
                </div>
                <div>
                  <div style={{ display: 'inline-block', padding: '3px 12px', marginBottom: '6px', background: categoryColors[selectedEvent.category] || 'rgba(201,168,76,0.1)', border: `1px solid ${categoryBorders[selectedEvent.category] || 'rgba(201,168,76,0.3)'}`, fontFamily: 'Cinzel, serif', fontSize: '0.58rem', letterSpacing: '0.18em', color: 'var(--gold)' }}>{selectedEvent.category}</div>
            ...
            <div style={{ padding: '2rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                {[
                  ...
                  { icon: '🕐', label: 'Time', value: selectedEvent.time || 'TBA' },
                  { icon: '📍', label: 'Venue', value: selectedEvent.venue || 'TBA' },
                  ...
                  <div key={item.label} style={{ padding: '1rem', background: 'var(--deep)', border: '1px solid var(--border)' }}>
                    <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.58rem', letterSpacing: '0.2em', color: 'var(--gold)', marginBottom: '5px' }}>{item.icon} {item.label.toUpperCase()}</div>
              ...
              {selectedEvent.description && (
                <div style={{ marginBottom: '2rem' }}>
                  ...
                  <p style={{ color: 'var(--muted)', lineHeight: 1.9, fontSize: '1rem', fontStyle: 'italic' }}>{selectedEvent.description}</p>
                </div>
              )}
              {selectedEvent.tickets_available && selectedEvent.ticket_url ? (
                <div style={{ padding: '1.5rem', background: 'rgba(201,168,76,0.05)', border: '1px solid rgba(201,168,76,0.25)', marginBottom: '1rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                    <div>
                      <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.65rem', letterSpacing: '0.2em', color: 'var(--gold)', marginBottom: '4px' }}>🎟 TICKETS AVAILABLE</div>
                      <p style={{ color: 'var(--muted)', fontSize: '0.85rem', fontStyle: 'italic' }}>Secure your seat via TickeTora</p>
                    </div>
                    <TicketLink url={selectedEvent.ticket_url} style={{ padding: '0.85rem 2.5rem', background: 'var(--gold)', color: 'var(--deep)', textDecoration: 'none', fontFamily: 'Cinzel, serif', fontSize: '0.72rem', letterSpacing: '0.2em', fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: '0.5rem', transition: 'all 0.3s' }}>
                ...
                <div style={{ padding: '1.25rem 1.5rem', background: 'rgba(76,201,130,0.05)', border: '1px solid rgba(76,201,130,0.2)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.65rem', letterSpacing: '0.2em', color: 'rgba(76,201,130,0.8)', marginBottom: '3px' }}>FREE ENTRY</div>
                    <p style={{ color: 'var(--muted)', fontSize: '0.85rem', fontStyle: 'italic' }}>This event is free and open to everyone. Just come as you are!</p>
                  </div>
                </div>
              )}
            </div>
