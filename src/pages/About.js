...
const hardcodedMembers = [
  {
    ...
    fun_fact: 'Belting out tunes in the shower and splendid creation of negative harmonies',
  },
  {
    id: 'rumbi',
    ...
    fun_fact: 'Rumbi can harmonise to almost any song within seconds of hearing it — the group calls it her "superpower".',
  },
  {
    id: 'kuda',
    ...
    fun_fact: "There is a huge dichotomy between Kuda's talking voice and his singing voice.",
  },
  {
    id: 'tanaka',
    ...
    fun_fact: 'Tanaka still uses the same bass guitar he bought secondhand for $15 at age 16.',
  },
  {
    id: 'tino',
    ...
    fun_fact: 'Joined Sanctified after listening to a touching song "Hupenyu" at CUTASA services.',
  },
  {
    id: 'clive',
    ...
    fun_fact: 'Clive has a unique ability to "mid-lock" his voice while singing; however, due to his vocal history, he has a defined limit and cannot push his range beyond that specific locking point.',
  },
  {
    id: 'runya',
    ...
    fun_fact: 'Runya once performed at a concert just 3 hours after finishing a 12-hour hospital shift.',
  },
  {
    id: 'brendon',
    name: 'Bredon',
    ...
    fun_fact: 'Brendon speaks 4 languages and can sing in all of them — including a full gospel song in Swahili.',
  },
  {
    id: 'fadzi',
    ...
    fun_fact: 'Fadzi runs a free music class for underprivileged children every Saturday morning.',
  },
  {
    id: 'joina',
    ...
    interests: 'Long distance travelling, Music and singing, Inquisitive in a lot especially food and culinary work',
    fun_fact: 'Joina has a love and hate relationship with avocados, and has always been like that since her childhood according to her father. She is a huge fan of sleep ASMR and chicken burgers. Some wonder why she would love chicken burgers but absolutely hate beef burgers, interesting!!',
  },
  {
    id: 'sibo',
    ...
    fun_fact: "Sibo choreographed the group's most popular performance in just one afternoon.",
  },
  {
    id: 'tanya',
    ...
    values: 'Expression, freedom in worship, and the power of the body as an instrument of praise.',
    interests: 'Dance, fashion, motivational speaking, and community outreach.',
    fun_fact: "Tanya choreographed the group's most popular performance in just one afternoon.",
  },
];
...
export default function About() {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      ...
      // Only replace hardcoded members if Supabase has data
    ...
    <div style={{ paddingTop: '75px', minHeight: '100vh' }}>
      ...
      <section style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          <span style={{ fontFamily: 'Cinzel, serif', fontSize: '0.65rem', letterSpacing: '0.5em', color: 'var(--gold)' }}>OUR STORY</span>
          <div style={{ width: '40px', height: '1px', background: 'var(--gold)' }} />
        </div>
        ...
        <p style={{ maxWidth: '650px', margin: '0 auto', color: 'var(--muted)', fontStyle: 'italic', fontSize: '1.15rem', lineHeight: 2 }}>
      ...
      <section style={{ padding: '6rem 2rem', maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'center' }} className="story-grid">
          <div>
            ...
            <p style={{ color: 'var(--muted)', lineHeight: 2, fontSize: '1rem', marginBottom: '1.2rem' }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ padding: '3rem 2.5rem', border: '1px solid var(--border)', background: 'var(--card)', position: 'relative', textAlign: 'center' }}>
            ...
            <div style={{ position: 'absolute', bottom: '-1.5rem', right: '-1.5rem', background: 'rgba(201,168,76,0.1)', border: '1px solid var(--border)', padding: '1.25rem 1.75rem', zIndex: 2, textAlign: 'center' }}>
      ...
      {/* ═══ VALUES ═══ */}
      <section style={{ padding: '5rem 2rem', background: 'linear-gradient(180deg, transparent, rgba(201,168,76,0.04), transparent)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
            {coreValues.map(v => (
              <div key={v.title} style={{ padding: '2.5rem 1.5rem', textAlign: 'center', background: 'var(--card)', border: '1px solid var(--border)', transition: 'border-color 0.3s, transform 0.3s' }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1.5rem' }}>
            {members.map((member, i) => (
              <div
                ...
                style={{ cursor: 'pointer', position: 'relative', overflow: 'hidden', background: 'var(--card)', border: '1px solid var(--border)', transition: 'all 0.35s ease', aspectRatio: '3/4' }}
                onMouseOver={e => {
              ...
              >
                ...
                />
                <div style={{ display: 'none', width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', background: `linear-gradient(135deg, rgba(201,168,76,${0.05 + i * 0.02}), var(--card2))`, position: 'absolute', inset: 0, fontFamily: 'Cinzel, serif', fontSize: '4rem', color: 'rgba(201,168,76,0.3)' }}>
                  {member.name.charAt(0)}
                </div>
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to top, rgba(8,8,16,0.97) 0%, rgba(8,8,16,0.7) 60%, transparent 100%)', padding: '2rem 1.25rem 1.25rem' }}>
                  <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.6rem', letterSpacing: '0.25em', color: 'var(--gold)', marginBottom: '4px' }}>"{member.nickname}"</div>
                  <h3 style={{ color: 'var(--text)', fontSize: '1rem', marginBottom: '2px' }}>{member.name}</h3>
                  <p style={{ color: 'var(--muted)', fontSize: '0.78rem', fontStyle: 'italic', letterSpacing: '0.05em' }}>{member.role}</p>
                </div>
                <div className="member-overlay" style={{ position: 'absolute', inset: 0, background: 'rgba(201,168,76,0.08)', border: '2px solid rgba(201,168,76,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity 0.3s ease' }}>
                  <div style={{ background: 'rgba(8,8,16,0.85)', padding: '0.6rem 1.5rem', fontFamily: 'Cinzel, serif', fontSize: '0.65rem', letterSpacing: '0.25em', color: 'var(--gold)', border: '1px solid rgba(201,168,76,0.4)' }}>VIEW BIO</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ═══ BIO MODAL ═══ */}
      {selected && (
        <div onClick={() => setSelected(null)} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(8,8,16,0.92)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', overflowY: 'auto' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--card)', border: '1px solid rgba(201,168,76,0.3)', maxWidth: '750px', width: '100%', position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}>
            <button onClick={() => setSelected(null)} style={{ position: 'sticky', top: '1rem', float: 'right', marginRight: '1rem', marginTop: '1rem', background: 'rgba(201,168,76,0.1)', border: '1px solid var(--border)', color: 'var(--gold)', cursor: 'pointer', width: '36px', height: '36px', fontSize: '1.1rem', fontFamily: 'Cinzel, serif', zIndex: 10 }}>✕</button>
            <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr' }} className="modal-grid">
              <div style={{ position: 'relative', minHeight: '300px' }}>
                <img
                  src={selected.image_url || selected.image}
                  alt={selected.name}
                  style={{ width: '100%', height: '100%', minHeight: '300px', objectFit: 'cover', objectPosition: 'center top', display: 'block' }}
                  onError={e => {
                    e.target.style.display = 'none';
                    e.target.nextSibling.style.display = 'flex';
                  }}
                />
                <div style={{ display: 'none', width: '100%', height: '300px', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, rgba(201,168,76,0.1), var(--dark))', fontFamily: 'Cinzel, serif', fontSize: '5rem', color: 'rgba(201,168,76,0.3)' }}>
                  {selected.name.charAt(0)}
                </div>
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to top, var(--card), transparent)', height: '80px' }} />
              </div>
              <div style={{ padding: '2rem 2rem 2rem 1.5rem' }}>
                <div style={{ marginBottom: '1.5rem', paddingRight: '2.5rem' }}>
                  <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.62rem', letterSpacing: '0.3em', color: 'var(--gold)', marginBottom: '6px' }}>"{selected.nickname}"</div>
                  <h2 style={{ fontSize: '1.6rem', color: 'var(--text)', marginBottom: '4px', lineHeight: 1.2 }}>{selected.name}</h2>
                  <p style={{ color: 'var(--gold)', fontFamily: 'Cinzel, serif', fontSize: '0.7rem', letterSpacing: '0.15em' }}>{selected.role}</p>
                </div>
                ...
                {[
                  ...
                  { label: '😄 Fun Fact', content: selected.fun_fact || selected.funFact },
                ].map(section => section.content ? (
                  <div key={section.label} style={{ marginBottom: '1.25rem' }}>
                    <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.65rem', letterSpacing: '0.2em', color: 'var(--gold)', marginBottom: '6px', textTransform: 'uppercase' }}>{section.label}</div>
                    <p style={{ color: 'var(--muted)', fontSize: '0.95rem', lineHeight: 1.85, fontStyle: 'italic', paddingLeft: '0.75rem', borderLeft: '2px solid rgba(201,168,76,0.2)' }}>{section.content}</p>
                  </div>
                ) : null)}
              </div>
            </div>
